...
CFrameResource::CFrameResource()
{
	...
	mConstantBuffers[static_cast<UINT>(CONSTANT_BUFFER_TYPE::BONE_OFFSET)] = std::make_unique<CConstantBuffer>();
	mConstantBuffers[static_cast<UINT>(CONSTANT_BUFFER_TYPE::BONE_OFFSET)]->Initialize(4, ALIGNED_SIZE(sizeof(Matrix) * SKINNED_ANIMATION_BONES) * BONE_TRANSFORM_COUNT);
