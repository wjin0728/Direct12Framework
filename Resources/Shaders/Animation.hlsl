...
VS_OUTPUT output = (VS_OUTPUT)0;
    ...
    float3 positionW, normalW, tangentW;
    ApplyBoneTransform(input.position, input.normal, input.tangent, input.boneIndices, input.boneWeights, positionW, normalW, tangentW);

    output.worldPos = mul(float4(positionW, 1.0f), worldMat);
    output.position = mul(output.worldPos, viewProjMat);

    output.worldNormal = mul(normalW, (float3x3)invWorldMat);
    output.worldTangent = mul(tangentW, (float3x3)invWorldMat);
    output.uv = input.uv;
