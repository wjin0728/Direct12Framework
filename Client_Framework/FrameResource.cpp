...
CFrameResource::CFrameResource()
{
	...
	mConstantBuffers[static_cast<UINT>(CONSTANT_BUFFER_TYPE::MATERIAL)]->Initialize(4, ALIGNED_SIZE(100) * MATERIAL_COUNT);	

	mStructedBuffers[static_cast<UINT>(STRUCTED_BUFFER_TYPE::UI)] = std::make_shared<CStructedBuffer>();
	mStructedBuffers[static_cast<UINT>(STRUCTED_BUFFER_TYPE::UI)]->Initialize(5, sizeof(CBUIData), 100, nullptr);
